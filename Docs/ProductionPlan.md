...
- [ ] **All 4 real Knight abilities** — not built yet. Only the throwaway test ability above exists so far. Build these next.
- [ ] Enhanced Input wired for the 4 abilities + movement, via a new `IMC_DC_Default` — the editor is open now, so this is no longer blocked; still needs doing.
