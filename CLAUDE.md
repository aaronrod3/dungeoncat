...
`Docs/GameDevPlan.md` is the design plan of record for the actual game (co-op ARPG dungeon crawler — GAS, dungeon generation technique currently reopened, bipedal cat, 2-month beta scope) — see the Project overview note below on how that relates to what's currently in this repo. `Docs/SystemsDesign.md` is the technical spec (naming convention, GAS/combat, replication, dungeon generation, AI, itemization, UI) that engineering builds against — content docs below own the creative "what," this owns the "how." `Docs/Classes.md`, `Docs/GameplayLoops.md`, and `Docs/Items.md` are ongoing living content docs (class kits/weapons/skills, the Hub World and core loop, the item/weapon/loot catalog) — start here for "what is actually in the game," not `SystemsDesign.md`. `Docs/AssetPipeline.md` is the Blender-facing spec (skeleton, sockets, animation list, naming) for self-authored art. `Docs/ProductionPlan.md` is a lightweight index into per-phase detail docs (`Docs/P2_DungeonAI.md`, `Docs/P3_CoopHardening.md`, `Docs/P4_LoopAndBoss.md`), each structured as plan-finalization → do-yourself → testing/manual stages. `Docs/CommandReference.md` has ready-to-paste build/git/backup/editor-close commands. `Docs/AsyncSessionProtocol.md` = behavioral protocol for a flagged "away session" (dev says something like "this is an away session") — read once at the start of one, not auto-loaded otherwise.
